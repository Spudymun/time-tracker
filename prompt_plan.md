...
Выполняй промпты по одному в **Copilot Agent Mode** (используй `@workspace` или Agent с MCP).
После каждого промпта: проверь что `npm run type-check` не выдаёт ошибок.
Отмечай выполненные пункты в `todo.md` по мере выполнения.
